...
for (colname in c("releases")) {
  ...
  if (length(zelda[[colname]]) > length(check_zelda[[colname]])) {
      cat(paste0("tibble in 2.RData has more rows than expected"))
      quit(status = 1)
  }

  if (!all(zelda[[colname]] == check_zelda[[colname]], na.rm = TRUE)) {
      cat(paste0("In ", colname, " column, tibble in 2.RData contains data that is out of order or different from what's expected"))
      quit(status = 1)
  }
}
